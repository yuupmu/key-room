...
export async function createChordScorePdf(bars, title, bpm, pdfLib) {
  check(Array.isArray(bars) && bars.length >= 1 && bars.length <= 100, '코드 악보는 1~100마디까지 만들 수 있습니다.');
  check(Number.isInteger(bpm) && bpm >= 30 && bpm <= 300, 'BPM은 30~300으로 입력해 주세요.');
  const { PDFDocument, StandardFonts, rgb } = pdfLib;
  const document = await PDFDocument.create();
  const font = await document.embedFont(StandardFonts.Helvetica);
  const bold = await document.embedFont(StandardFonts.HelveticaBold);
  const safeTitle = printable(title, 'Chord Score');
  document.setTitle(String(title || 'Chord Score').slice(0, 100));
  const ink = rgb(0.1, 0.13, 0.17), muted = rgb(0.38, 0.43, 0.49);
  const barsPerSystem = 4, systemsPerPage = 4;
  const systemCount = Math.ceil(bars.length / barsPerSystem);
  const pageCount = Math.ceil(systemCount / systemsPerPage);
  const barWidth = 486 / barsPerSystem;
  for (let system = 0; system < systemCount; system++) {
    const pageIndex = Math.floor(system / systemsPerPage);
    const page = document.getPages()[pageIndex] || document.addPage([595.28, 841.89]);
    if (system % systemsPerPage === 0) {
      page.drawText(safeTitle, { x: 38, y: 804, size: 16, font: bold, color: ink, maxWidth: 480 });
      page.drawText(`4/4  |  ${bpm} BPM`, { x: 38, y: 785, size: 8, font, color: muted });
      page.drawText(`${pageIndex + 1} / ${pageCount}`, { x: 524, y: 28, size: 8, font, color: muted });
    }
    const top = 752 - system % systemsPerPage * 178;
    const trebleBottom = top - 43, bassBottom = top - 113;
    drawStaff(page, pdfLib, 76, 562, trebleBottom);
    drawStaff(page, pdfLib, 76, 562, bassBottom);
    page.drawText('G', { x: 53, y: trebleBottom + 8, size: 18, font: bold, color: ink });
    page.drawText('F', { x: 53, y: bassBottom + 8, size: 18, font: bold, color: ink });
    const startBar = system * barsPerSystem;
    const endBar = Math.min(bars.length, startBar + barsPerSystem);
    for (let barIndex = startBar; barIndex < endBar; barIndex++) {
      const bar = bars[barIndex], left = 76 + (barIndex - startBar) * barWidth, right = left + barWidth;
      page.drawText(String(bar.number), { x: left + 3, y: top + 19, size: 7, font, color: muted });
      page.drawLine({ start: { x: right, y: bassBottom }, end: { x: right, y: trebleBottom + 32 }, color: ink, thickness: 0.9 });
      for (const symbol of bar.symbols) {
        const x = left + 12 + symbol.beat / 4 * (barWidth - 24);
        page.drawText(printable(symbol.text, '?'), { x, y: top + 5, size: 8, font: bold, color: ink, maxWidth: Math.max(18, barWidth - (x - left) - 4) });
      }
      for (const [events, bottom, clef] of [[bar.right, trebleBottom, 'G'], [bar.left, bassBottom, 'F']]) {
        const accidentals = new Map();
        for (const event of events) {
          const x = left + 12 + event.beat / 4 * (barWidth - 24);
          if (!event.notes.length) { drawRest(page, pdfLib, x, bottom); continue; }
          for (const note of event.notes) {
            const written = spell(note.midi, 0), id = `${written.step}:${written.octave}`;
            const previous = accidentals.has(id) ? accidentals.get(id) : 0;
            const accidental = previous === written.alter ? '' : ({ '-2': 'bb', '-1': 'b', 0: 'n', 1: '#', 2: 'x' })[written.alter];
            accidentals.set(id, written.alter);
            drawNote(page, pdfLib, font, { pitch: note.midi }, x, bottom, clef, 0, event.length, false, false, accidental);
          }
        }
      }
    }
